...
export const ProjectsView: React.FC = () => {
  ...
  return <main className="flex-1 w-full max-w-5xl mx-auto overflow-y-auto p-4 sm:p-6 space-y-6">
    <header><h1 className="flex items-center gap-2 text-xl font-bold text-white"><FolderKanban className="text-sky-400" /> {t('projects.title')}</h1><p className="mt-1 text-sm text-slate-400">{t('projects.subtitle')}</p></header>
    <form onSubmit={create} className="space-y-3 rounded-2xl border border-slate-800 bg-slate-900 p-5"><h2 className="font-semibold text-white">{t('projects.create')}</h2><input className="field" value={name} onChange={(event) => setName(event.target.value)} placeholder={t('projects.name')} maxLength={100} required /><textarea className="field" value={description} onChange={(event) => setDescription(event.target.value)} placeholder={t('projects.description')} maxLength={2000} rows={3} /><button className="btn bg-sky-600 hover:bg-sky-500" type="submit"><Plus className="h-4 w-4" /> {t('projects.create')}</button></form>
    {status && <p className="text-sm text-amber-300" role="status">{status}</p>}
    <section className="grid gap-3 sm:grid-cols-2">{projects.length === 0 && <p className="text-sm text-slate-500">{t('projects.empty')}</p>}{projects.map((project) => <article key={project.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold text-slate-100">{project.name}</h2><span className="mt-2 inline-block rounded-full border border-emerald-800 px-2 py-0.5 text-[11px] text-emerald-400">{project.status}</span></div><button aria-label={`Delete ${project.name}`} className="rounded-lg p-2 text-rose-400 hover:bg-rose-950" onClick={() => remove(project)}><Trash2 className="h-4 w-4" /></button></div><p className="mt-4 text-sm leading-relaxed text-slate-400">{project.description || t('projects.description')}</p><p className="mt-4 text-xs text-slate-600">Updated {new Date(project.updatedAt).toLocaleDateString()}</p></article>)}</section>
  </main>;
};
