...
export const AccountMenu: React.FC<{ auth: AuthState; onLoggedOut: () => void }> = ({ auth, onLoggedOut }) => {
  ...
  return <div className="relative">
    ...
    {open && <div className="absolute right-0 mt-2 w-52 rounded-xl border border-slate-800 bg-slate-900 p-2 shadow-2xl z-50">
      <div className="px-3 py-2 border-b border-slate-800 mb-1"><p className="text-sm text-slate-200 truncate">{auth.user.name}</p><p className="text-xs text-slate-500 truncate">{auth.user.email || t('common.githubAccount')}</p></div>
      <button onClick={async () => { await api.logout(); onLoggedOut(); }} className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-white"><LogOut className="w-4 h-4" /> {t('common.signOut')}</button>
    </div>}
  </div>;
};
