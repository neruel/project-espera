...
export const SettingsView: React.FC<Props> = ({ session, onUpdateSession, providers }) => {
  ...
  return <main className="flex-1 w-full max-w-5xl mx-auto overflow-y-auto p-4 sm:p-6 space-y-6">
    ...
    <section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900 p-5"><h2 className="font-semibold text-white">{t('settings.addConnection')}</h2><div className="grid gap-4 sm:grid-cols-2">
      <label className="text-sm text-slate-300">{t('settings.provider')}<select className="field" value={providerId} onChange={(event) => changeProvider(event.target.value)}>{providers.map((provider) => <option key={provider.id} value={provider.id}>{provider.name}</option>)}</select></label>
      <label className="text-sm text-slate-300">{t('settings.connectionName')}<input className="field" value={name} onChange={(event) => setName(event.target.value)} /></label>
      <label className="text-sm text-slate-300 sm:col-span-2">{t('settings.baseUrl')}<input className="field font-mono" value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} placeholder="https://api.example.com/v1" /></label>
      <label className="text-sm text-slate-300 sm:col-span-2"><Key className="mr-1 inline h-3 w-3" /> {t('settings.apiKey')}<input type="password" autoComplete="off" className="field font-mono" value={key} onChange={(event) => setKey(event.target.value)} placeholder={providerId === 'mock' ? 'Not required for Mock' : 'Required for discovery and chat'} /></label>
    </div><div className="flex flex-wrap gap-2"><button className="btn" disabled={busy || (!key && providerId !== 'mock')} onClick={discoverModels}><RefreshCw className={busy ? 'h-4 w-4 animate-spin' : 'h-4 w-4'} /> {t('settings.discover')}</button></div>
