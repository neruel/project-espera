...
- Worker `project-espera-api` and Pages project `project-espera-web` run the `1.0.0-beta.2` build.
...
- The production endpoint policy is `allowlisted-https`: official APIs and the configured OpenAI-compatible API hosts are available over HTTPS/443. Other custom hosts require operator review and configuration.
